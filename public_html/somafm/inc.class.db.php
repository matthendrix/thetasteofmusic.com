	...
	*/
	public function query($query, $bindparams = array(), $echo = false) {
		...
		//not prepared
			...
			$types = array('SELECT', 'INSERT', 'DELETE', 'UPDATE', 'CREATE', 'DROP', 'TRUNCATE', 'SET', 'SHOW', 'REPLACE');
			$type = strtok($query, " \r\n\t");
			$type_upper = strtoupper($type);
			if(!in_array($type_upper, $types)) {
				$this->errorlog('query type invalid. Query was: '. $query, 1);
			}
			elseif($type_upper == 'SELECT') {
				try{
